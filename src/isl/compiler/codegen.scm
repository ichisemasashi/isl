...
(define (emit-module units include-main?)
  ...
  (let* ((tops (collect-ll-tops units))
         ...
         (decls (if include-main?
                    ...
                    (module-decls)))
         ;; emit-aot-main may intern additional strings (symbol names for top-level registration),
         ;; so build main first and snapshot constants after all codegen passes.
