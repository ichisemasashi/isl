...
(define (install-primitives! env)
  ...
  (def 'listp
    (lambda (args state)
      ...
      (host->runtime-value (if (list? (runtime-value->host (car args))) #t #f))))
  (def 'null
    (lambda (args state)
      (unless (= (length args) 1)
        (runtime-raise 'arity "null expects 1 argument" args))
      (host->runtime-value (if (null? (runtime-value->host (car args))) #t #f))))
  (def 'car
    (lambda (args state)
      (unless (= (length args) 1)
  ...
  (def 'length
    (lambda (args state)
      ...
      (let ((v (runtime-value->host (car args))))
        (if (or (list? v) (vector? v) (string? v))
            (host->runtime-value (length v))
            (runtime-raise 'type-error "length expects list/vector/string" v)))))
  (def 'first
    (lambda (args state)
      (unless (= (length args) 1)
       ...
       (let ((let-env (make-env env)))
         (for-each
          (lambda (b)
            (unless (and (list? b) (= (length b) 2) (symbol? (car b)))
              (runtime-raise 'invalid-ir "let* binding must be (symbol expr)" b))
            (let ((v (runtime-eval-expr (cadr b) let-env state)))
              (env-define! let-env (car b) v)))
          bindings)
         (runtime-eval-expr body let-env state))))
    ((setf)
     (unless (and (list? payload) (= (length payload) 2))
