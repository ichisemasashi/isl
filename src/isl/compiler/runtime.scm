...
(define (install-primitives! env)
  ...
  (def 'car
    (lambda (args state)
      ...
      (let ((v (runtime-value->host (car args))))
        (unless (pair? v)
          (runtime-raise 'type-error "car expects pair" v))
  ...
  (def 'cdr
    (lambda (args state)
      ...
      (let ((v (runtime-value->host (car args))))
        (unless (pair? v)
          (runtime-raise 'type-error "cdr expects pair" v))
        (host->runtime-value (cdr v)))))
  (def 'cons
    (lambda (args state)
      (unless (= (length args) 2)
  ...
  (def 'substring
    (lambda (args state)
      (unless (= (length args) 3)
        (runtime-raise 'arity "substring expects 3 arguments" args))
      (let ((s (runtime-string (car args) "substring"))
            (start (runtime-integer (cadr args) "substring"))
            (end (runtime-integer (caddr args) "substring")))
