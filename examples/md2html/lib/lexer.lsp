...
(defun line-left-trim-index (line)
  (let ((i 0)
        (n (length line)))
    (while (and (< i n)
                (not (null (string-index (substring line i (+ i 1)) " \t"))))
      (setq i (+ i 1)))
    i))

(defun blockquote-content-or-nil (line)
  (let* ((n (length line))
         (i (line-left-trim-index line)))
  ...
  (let ((lines (split-lines text))
    ...
    (dolist (line lines)
      (if (line-blank-p line)
          ...
          (let ((q (blockquote-content-or-nil line)))
            (if (null q)
                ...
                (setq tokens (append tokens (list (make-line-token 'quote q (md-pos line-no 1 offset) '())))))))
         ...
         ((string= ch "!")
          (if (> (length buf) 0)
              (progn
                ...
                (setq buf ""))
              nil)
          (setq tokens (append tokens (list (make-inline-token 'bang "!" (md-pos (md-pos-line pos) col offset)))))
          (setq i (+ i 1))
          (setq col (+ col 1))
          (setq offset (+ offset 1))
          (setq buf-col col)
