            ...
            (if (null cap)
                ...
                (let ((label-raw (first cap))
                  ...
                  (if (starts-with label-raw "^")
                      (progn
                        (setq nodes (append nodes (list (make-footnote-ref-node (substring label-raw 1 (length label-raw)) (inline-token-pos tok)))))
                        (setq rest after-rbrack))
                      (let ((caret-res (strip-trailing-caret-from-nodes nodes))
                        ...
                        (if (second caret-res)
                            (progn
                              (setq nodes (append (first caret-res) (list (make-inline-note-node label-raw (inline-token-pos tok)))))
                              (setq rest after-rbrack))
                            (if (null citation-node)
                              ...
                              (if (first tail-res)
                                  ...
                                  (if (and (not (null tail-after-label))
                                      ...
                                      (progn
                                        ...
                                        (setq rest (cdr rest))))))
                                (progn
                                  (setq nodes (append nodes (list citation-node)))
                                  (setq rest after-rbrack))))))))))
         (t
          (setq nodes (append nodes (list (make-inline-text (inline-token-pos tok) (inline-token-lexeme tok)))))
          (setq rest (cdr rest))))))
