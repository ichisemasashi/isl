...
(defmacro md-push-front! (place value)
  `(setq ,place (cons ,value ,place)))

(defun flush-para-state-rev (blocks-rev para-lines-rev para-pos)
  (if (null para-lines-rev)
      (list blocks-rev para-lines-rev para-pos)
      (list (cons (make-block-paragraph para-pos
                                        (parse-inline (join-lines-newline (md-reverse para-lines-rev)) para-pos))
                  blocks-rev)
            '()
            '())))

(defun consume-parsed-block-rev (blocks-rev parsed)
  (list (cons (first parsed) blocks-rev)
        (second parsed)))
  ...
  (let ((rest line-tokens)
    ...
    (while (not (null rest))
      (let ((tok (first rest)))
        (cond
         ((eq (line-token-kind tok) 'blank)
          (let ((state (flush-para-state-rev blocks-rev para-lines-rev para-pos)))
            (setq blocks-rev (first state))
            (setq para-lines-rev (second state))
          ...
          (if (or prev-blank at-start)
              (progn
                ...
                (let* ((meta (heading-meta (line-token-text tok)))
                       ...
                       (hclasses (third meta))
                       (hattrs (fourth meta)))
                  (md-push-front! blocks-rev (make-block-heading
                                              (line-token-pos tok)
              ...
              (progn
                ...
                (md-push-front! para-lines-rev
                                (string-append (repeat-string "#" (line-token-detail tok))
                                               " "
                                               (line-token-text tok)))
                (setq rest (cdr rest))))
          (setq prev-blank nil)
          ...
          (if (and (literate-haskell-enabled-p) (or prev-blank at-start))
              (let ((state (flush-para-state-rev blocks-rev para-lines-rev para-pos))
                ...
                (while (and (not (null qrest)) (eq (line-token-kind (first qrest)) 'quote))
                  (md-push-front! clines-rev (line-token-text (first qrest)))
                  (setq qrest (cdr qrest)))
                (md-push-front! blocks-rev
                                (make-block-code cpos
                                                 (join-lines-newline (md-reverse clines-rev))
                                                 '()
                                                 '()
                                                 '()))
                (setq rest qrest))
              (if (or prev-blank at-start)
                  (let ((state (flush-para-state-rev blocks-rev para-lines-rev para-pos))
                    ...
                    (setq para-pos (third state))
                    (while (and (not (null qrest)) (eq (line-token-kind (first qrest)) 'quote))
                      (md-push-front! qlines-rev (line-token-text (first qrest)))
                      (setq qrest (cdr qrest)))
                    (md-push-front! blocks-rev
                                    (make-block-blockquote qpos
                                                           (parse-markdown (join-lines-newline (md-reverse qlines-rev)))))
                    (setq rest qrest))
                  (progn
                    (if (null para-pos) (setq para-pos (line-token-pos tok)) nil)
                    (md-push-front! para-lines-rev (string-append "> " (line-token-text tok)))
                    (setq rest (cdr rest)))))
          (setq prev-blank nil)
          (setq at-start nil))
         (t
          (let* ((line (line-token-text tok))
                 ...
                 (line-block-content (if (eq (line-token-kind tok) 'paragraph)
                                         ...
                                         '()))
                 (handled nil))
            (if (and (not handled) (not (null lvl)) (not (null para-lines-rev)))
                (progn
                  (let* ((meta (heading-meta (join-lines-space (md-reverse para-lines-rev))))
                         (htext (first meta))
                         (hid (second meta))
                         (hclasses (third meta))
                         (hattrs (fourth meta)))
                    (md-push-front! blocks-rev
                                    (make-block-heading para-pos
                                                        lvl
                  ...
                  (setq handled t))
                nil)
            (if (and (not handled) (not (null def-content)) (not (null para-lines-rev)))
                (let ((parsed-def (parse-definition-block rest (md-reverse para-lines-rev) para-pos))
                      (state '()))
                  (setq state (consume-parsed-block-rev blocks-rev parsed-def))
                  (setq blocks-rev (first state))
                  (setq rest (second state))
                  (setq para-lines-rev '())
                  (setq para-pos '())
                  (setq handled t))
                nil)
            (if (and (not handled) (null para-lines-rev) (not (null fence-info)))
                (let ((parsed-fence (parse-fenced-code-block rest fence-info))
                      (state '()))
                  (setq state (consume-parsed-block-rev blocks-rev parsed-fence))
                  (setq blocks-rev (first state))
                  (setq rest (second state))
                  (setq handled t))
                nil)
            (if (and (not handled) (null para-lines-rev) (not (null fenced-div-meta)))
                (let ((parsed-div (parse-fenced-div-block rest fenced-div-meta))
                      (state '()))
                  (setq state (consume-parsed-block-rev blocks-rev parsed-div))
                  (setq blocks-rev (first state))
                  (setq rest (second state))
                  (setq handled t))
                nil)
            (if (and (not handled) (null para-lines-rev) (not (null native-div-meta)))
                (let ((parsed-div (parse-native-div-block rest native-div-meta))
                  ...
                  (setq handled t))
                nil)
            (if (and (not handled) (null para-lines-rev) (not (null math-dollar)))
                (progn
                  (md-push-front! blocks-rev (first math-dollar))
                  (setq rest (second math-dollar))
                  (setq handled t))
                nil)
            (if (and (not handled) (null para-lines-rev) (not (null math-bracket1)))
                (progn
                  (md-push-front! blocks-rev (first math-bracket1))
                  (setq rest (second math-bracket1))
                  (setq handled t))
                nil)
            (if (and (not handled) (null para-lines-rev) (not (null math-bracket2)))
                (progn
                  (md-push-front! blocks-rev (first math-bracket2))
                  (setq rest (second math-bracket2))
                  (setq handled t))
                nil)
            (if (and (not handled) (null para-lines-rev) (not (null raw-html)))
                (progn
                  (md-push-front! blocks-rev (first raw-html))
                  (setq rest (second raw-html))
                  (setq handled t))
                nil)
            (if (and (not handled) (null para-lines-rev) (not (null raw-tex)))
                (progn
                  (md-push-front! blocks-rev (first raw-tex))
            ...
            (if (and (not handled)
                ...
                (let ((parsed-indent (parse-indented-code-block rest))
                      (state '()))
                  (setq state (consume-parsed-block-rev blocks-rev parsed-indent))
                  (setq blocks-rev (first state))
                  (setq rest (second state))
                  (setq handled t))
                nil)
            (if (and (not handled) (null para-lines-rev))
                (let ((parsed-table (parse-any-table-block rest)))
                  (if (null parsed-table)
                      nil
                      (let ((state (consume-parsed-block-rev blocks-rev parsed-table)))
                        (setq blocks-rev (first state))
                        (setq rest (second state))
                        (setq handled t))))
                nil)
            (if (and (not handled) (null para-lines-rev) (not (null line-block-content)))
                (let ((parsed-line-block (parse-line-block rest))
                      (state '()))
                  (setq state (consume-parsed-block-rev blocks-rev parsed-line-block))
                  (setq blocks-rev (first state))
                  (setq rest (second state))
                  (setq handled t))
                nil)
            (if (and (not handled) (not (null list-info)))
                (let ((state (flush-para-state-rev blocks-rev para-lines-rev para-pos)))
                  ...
                  (let ((parsed-list (parse-list-block rest list-info))
                        (next-state '()))
                    (setq next-state (consume-parsed-block-rev blocks-rev parsed-list))
                    (setq blocks-rev (first next-state))
                    (setq rest (second next-state)))
                  (setq handled t))
                nil)
            (if (and (not handled) (thematic-break-p line) (null para-lines-rev))
                (progn
                  (md-push-front! blocks-rev (make-block-hr (line-token-pos tok)))
                  (setq rest (cdr rest))
                  (setq handled t))
                nil)
            (if (not handled)
                (progn
                  (if (null para-pos) (setq para-pos (line-token-pos tok)) nil)
                  (md-push-front! para-lines-rev line)
                  (setq rest (cdr rest)))
                nil)
            (setq prev-blank nil)
            (setq at-start nil))))))
