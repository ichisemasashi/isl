...
- 対応範囲: HTTP/1.1, HTTPS, Linux/FreeBSD, 設定ファイル指定ポート, 同時接続100, 設定可能ドキュメントルート, CGI
- 完了定義 (DoD):
  - 全タスクの受け入れ条件を満たす
  - 自動テストが成功する
...
- 受け入れ条件:
  ...
  - `listen_port` が `1..65535` の範囲外の場合は起動失敗する

### T2. HTTP/1.1 サーバー基盤実装

- 実装内容:
  - 設定ファイルの `listen_port` で待ち受ける HTTP/1.1 サーバーを実装する
