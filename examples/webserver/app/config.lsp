...
(defun ws-validate-config-values (cfg)
  (let* ((listen-port (ws-require-key cfg 'listen_port))
         ...
         (pid-file-raw (ws-config-get cfg 'pid_file))
         (server-log-raw (ws-config-get cfg 'server_log))
         (access-log-raw (ws-config-get cfg 'access_log))
         (error-log-raw (ws-config-get cfg 'error_log))
         (transport-raw (ws-config-get cfg 'transport))
         (daemon-raw (ws-config-get cfg 'daemon))
         (cgi-timeout-raw (ws-config-get cfg 'cgi_timeout_sec))
         ...
         (pid-file (if (null pid-file-raw) "/tmp/webserver.pid" (ws-normalize-path pid-file-raw)))
         (server-log (if (null server-log-raw) "/tmp/webserver.log" (ws-normalize-path server-log-raw)))
         (access-log (if (null access-log-raw) "/tmp/webserver-access.log" (ws-normalize-path access-log-raw)))
         (error-log (if (null error-log-raw) "/tmp/webserver-error.log" (ws-normalize-path error-log-raw)))
         (transport (if (null transport-raw) "http" transport-raw))
         (daemon (if (null daemon-raw) #t daemon-raw))
         (cgi-timeout-sec (if (null cgi-timeout-raw) 30 cgi-timeout-raw))
    ...
    (if (not (or (eq cgi-enabled #t) (eq cgi-enabled #f) (null cgi-enabled)))
        ...
        nil)
    (if (not (stringp transport))
        (ws-die (format nil "transport must be string: ~A" transport))
        nil)
    (if (not (or (string= transport "http") (string= transport "https")))
        (ws-die (format nil "transport must be \"http\" or \"https\": ~A" transport))
        nil)
    (if (not (or (eq daemon #t) (eq daemon #f)))
        (ws-die (format nil "daemon must be #t or #f: ~A" daemon))
        nil)
    (if (not (ws-integer-value-p cgi-timeout-sec))
        (ws-die (format nil "cgi_timeout_sec must be integer: ~A" cgi-timeout-sec))
        nil)
    (if (< cgi-timeout-sec 1)
        (ws-die (format nil "cgi_timeout_sec must be >= 1: ~A" cgi-timeout-sec))
        nil)
    ...
    (list
     ...
     (list 'pid_file pid-file)
     (list 'server_log server-log)
     (list 'access_log access-log)
     (list 'error_log error-log)
     (list 'transport transport)
     (list 'daemon daemon)
     (list 'cgi_timeout_sec cgi-timeout-sec)
...
(defun ws-print-config (cfg)
  ...
  (ws-log (format nil "pid_file=~A" (ws-config-get cfg 'pid_file)))
  (ws-log (format nil "server_log=~A" (ws-config-get cfg 'server_log)))
  (ws-log (format nil "access_log=~A" (ws-config-get cfg 'access_log)))
  (ws-log (format nil "error_log=~A" (ws-config-get cfg 'error_log)))
  (ws-log (format nil "transport=~A" (ws-config-get cfg 'transport)))
  (ws-log (format nil "daemon=~A" (if (ws-config-get cfg 'daemon) "true" "false")))
  (ws-log (format nil "cgi_timeout_sec=~A" (ws-config-get cfg 'cgi_timeout_sec))))
