...
(defun ws-env-or-empty (name)
  (let ((v (getenv name)))
    (if (null v) "" v)))

(defun ws-save-env-vars (names)
  (let ((rest names)
        (saved '()))
    (while (not (null rest))
      (setq saved (cons (list (car rest) (ws-env-or-empty (car rest))) saved))
      (setq rest (cdr rest)))
    (ws-reverse saved)))

(defun ws-restore-env-vars (saved)
  (let ((rest saved))
    (while (not (null rest))
      (let ((entry (car rest)))
...
(defun ws-with-env-vars (bindings thunk)
  (let ((saved (ws-save-env-vars (mapcar (lambda (entry) (first entry)) bindings))))
    (handler-case
      (progn
        (dolist (entry bindings)
          (setenv (first entry) (second entry)))
        (let ((result (funcall thunk)))
          (ws-restore-env-vars saved)
          result))
      (error (e)
        (ws-restore-env-vars saved)
        (error e)))))
...
(defun ws-request-body-length (headers)
  (let ((transfer-encoding (ws-header-get headers "transfer-encoding"))
        (len-header (ws-header-get headers "content-length")))
    (if (and (stringp transfer-encoding)
             (not (string= (ws-trim transfer-encoding) "")))
        (ws-protocol-error 501 "unsupported Transfer-Encoding")
        (let ((content-length (if (null len-header) 0 (ws-parse-int len-header))))
    ...
    (if (eq line 'eof)
        ...
        (catch 'ws-bad-request
          (let* ((parts (ws-parse-request-line line))
                 ...
                 (content-length (ws-request-body-length headers))
...
(defun ws-http-date-from-epoch (cfg epoch-sec)
  (os-date-http-from-epoch (ws-config-get cfg 'os_family) epoch-sec))

(defun ws-epoch-from-http-date (cfg http-date)
  (os-epoch-from-http-date (ws-config-get cfg 'os_family) http-date))
...
(defun ws-byte-length (text)
  (let* ((tmp (ws-temp-file "webserver-bytes" ".tmp"))
    ...
    (setq out (format nil "~A" (os-wc-c tmp)))
    (ws-delete-file-if-exists tmp)
    (setq n (ws-parse-int out))
    (if (null n)
        (length text)
        n)))
...
(defun ws-static-file-within-public-p (public-root file-path)
  (if (not (ws-file-readable-p file-path))
      #f
      (if (os-test-symlink-p file-path)
          #f
...
(defun ws-cgi-script-resolvable-p (cgi-bin-dir rel-path abs-script)
  (if (or (not (ws-starts-with abs-script (string-append cgi-bin-dir "/")))
      ...
      (let ((parts (ws-split-path rel-path))
        ...
        (if (not ok)
            #f
            (and (os-test-file-readable-p abs-script)
                 (os-file-executable-p abs-script)
                 (not (os-test-symlink-p abs-script)))))))
