...
(defglobal *dbms-tx-staged-table-deltas* '())
(defglobal *dbms-tx-staged-auth* '())
;; lock entry: (<table-name> <mode:S|X> <holders:(txid ...)>)
(defglobal *dbms-lock-table* '())
;; row lock entry: (<table-name> <row-key> <mode:S|X> <holders:(txid ...)>)
(defglobal *dbms-row-lock-table* '())
...
(defun dbms-engine-init ()
  (let ((recovered (dbms-storage-recover-from-wal))
    ...
    (setq loaded-auth (dbms-storage-load-auth-meta))
    (setq auth (dbms-auth-ensure-bootstrap loaded-auth))
    (if (not (equal auth loaded-auth))
        (dbms-storage-save-auth-meta auth)
        nil)
    (setq loaded-security (dbms-storage-load-security-meta))
    (setq security (dbms-security-ensure-bootstrap loaded-security auth))
    (if (not (equal security loaded-security))
...
(defun dbms-engine-reset-tx-state ()
  ...
  (setq *dbms-tx-staged-table-deltas* '())
  (setq *dbms-tx-staged-auth* '())
  (setq *dbms-lock-table* '())
  (setq *dbms-row-lock-table* '())
...
(defun dbms-row-lock-table-snapshot ()
  *dbms-row-lock-table*)
...
(defun dbms-row-lock-find-entry (table-name row-key entries)
  (if (null entries)
      '()
      (let ((e (car entries)))
        (if (and (string= (first e) table-name)
                 (string= (second e) row-key))
            e
            (dbms-row-lock-find-entry table-name row-key (cdr entries))))))

(defun dbms-row-lock-put-entry (table-name row-key mode holders entries)
  (if (null entries)
      (list (list table-name row-key mode holders))
      (let ((e (car entries)))
        (if (and (string= (first e) table-name)
                 (string= (second e) row-key))
            (cons (list table-name row-key mode holders) (cdr entries))
            (cons e (dbms-row-lock-put-entry table-name row-key mode holders (cdr entries)))))))

(defun dbms-row-lock-remove-entry (table-name row-key entries)
  (if (null entries)
      '()
      (let ((e (car entries)))
        (if (and (string= (first e) table-name)
                 (string= (second e) row-key))
            (dbms-row-lock-remove-entry table-name row-key (cdr entries))
            (cons e (dbms-row-lock-remove-entry table-name row-key (cdr entries)))))))

(defun dbms-row-lock-target-id (table-name row-key)
  (string-append table-name "::" row-key))

(defun dbms-row-lock-enabled-p ()
  (let ((v (getenv "DBMS_ENABLE_ROW_LOCKS")))
    (if (or (null v)
            (string= v "")
            (string= v "1")
            (string= v "true")
...
(defun dbms-row-lock-acquire (txid table-name row-key mode)
  (if (or (not (numberp txid)) (<= txid 0))
      (dbms-make-error 'dbms/invalid-representation "row lock acquire requires txid>0" txid)
      (if (or (null table-name) (not (stringp table-name)) (string= table-name ""))
          (dbms-make-error 'dbms/invalid-representation "row lock acquire requires table-name" table-name)
          (if (or (null row-key) (not (stringp row-key)) (string= row-key ""))
              (dbms-make-error 'dbms/invalid-representation "row lock acquire requires row-key" row-key)
              (let ((entry (dbms-row-lock-find-entry table-name row-key *dbms-row-lock-table*))
                    (target-id (dbms-row-lock-target-id table-name row-key)))
                (if (null entry)
                    (progn
                      (setq *dbms-row-lock-table* (dbms-row-lock-put-entry table-name row-key mode (list txid) *dbms-row-lock-table*))
                      (dbms-wfg-clear-tx txid)
                      t)
                    (let ((cur-mode (third entry))
                          (holders (fourth entry)))
                      (cond
                       ((eq mode 'S)
                        (if (eq cur-mode 'S)
                            (progn
                              (if (dbms-number-in-list-p txid holders)
                                  ...
                                  (setq holders (append holders (list txid))))
                              (setq *dbms-row-lock-table*
                                    (dbms-row-lock-put-entry table-name row-key 'S holders *dbms-row-lock-table*))
                              (dbms-wfg-clear-tx txid)
                              t)
                            (if (dbms-number-in-list-p txid holders)
                                (progn
                                  (dbms-wfg-clear-tx txid)
                                  t)
                                (dbms-lock-make-conflict-or-deadlock txid target-id holders "row S lock blocked by X lock"))))
                       ((eq mode 'X)
                        (if (eq cur-mode 'X)
                            (if (dbms-number-in-list-p txid holders)
                                (progn
                                  (dbms-wfg-clear-tx txid)
                                  t)
                                (dbms-lock-make-conflict-or-deadlock txid target-id holders "row X lock held by other tx"))
                            (if (dbms-lock-holders-only-tx-p txid holders)
                                (progn
                                  (setq *dbms-row-lock-table*
                                        (dbms-row-lock-put-entry table-name row-key 'X (list txid) *dbms-row-lock-table*))
                                  (dbms-wfg-clear-tx txid)
                                  t)
                                (dbms-lock-make-conflict-or-deadlock txid target-id holders "row X lock blocked by shared holders"))))
                       (t
                        (dbms-make-error 'dbms/invalid-representation "row lock mode must be S or X" mode))))))))))

(defun dbms-lock-release-tx (txid)
  (let ((entries *dbms-lock-table*)
        (out '())
        (row-entries *dbms-row-lock-table*)
        (row-out '()))
    ...
    (while (not (null row-entries))
      (let* ((e (car row-entries))
             (table-name (first e))
             (row-key (second e))
             (mode (third e))
             (holders (fourth e))
        ...
        (if (null new-holders)
            nil
            (setq row-out (append row-out (list (list table-name row-key mode new-holders)))))
        (setq row-entries (cdr row-entries))))
    (setq *dbms-lock-table* out)
    (setq *dbms-row-lock-table* row-out)
    (dbms-wfg-clear-tx txid)
    t))
...
(defun dbms-engine-row-lock-table-debug ()
  (dbms-row-lock-table-snapshot))
...
(defun dbms-find-staged-table-delta-pair (table-name staged-deltas)
  (if (null staged-deltas)
      '()
      (let ((h (car staged-deltas)))
        (if (string= (first h) table-name)
            h
            (dbms-find-staged-table-delta-pair table-name (cdr staged-deltas))))))

(defun dbms-put-staged-table-delta (table-name delta staged-deltas)
  (if (null staged-deltas)
      (list (list table-name delta))
      (let ((h (car staged-deltas)))
        (if (string= (first h) table-name)
            (cons (list table-name delta) (cdr staged-deltas))
            (cons h (dbms-put-staged-table-delta table-name delta (cdr staged-deltas)))))))
...
(defun dbms-engine-save-table-rows (table-name rows)
  (if (dbms-tx-active-p)
      (let* ((pair (dbms-find-staged-table-state-pair table-name *dbms-tx-staged-table-states*))
             ...
             (merged-ops (dbms-engine-merge-row-ops
                          (dbms-engine-table-delta-ops-or-empty table-name *dbms-tx-staged-table-deltas*)
                          step-ops))
             (state (dbms-make-table-state table-name rows (+ (length rows) 1))))
        (setq *dbms-tx-staged-table-states*
              (dbms-put-staged-table-state table-name state *dbms-tx-staged-table-states*))
        (setq *dbms-tx-staged-table-deltas*
              (dbms-put-staged-table-delta table-name merged-ops *dbms-tx-staged-table-deltas*))
...
(defun dbms-make-row-op (kind old-row new-row)
  (list kind old-row new-row))

(defun dbms-row-op-kind (op)
  (if (and (listp op) (>= (length op) 3))
      (first op)
      '()))

(defun dbms-row-op-old-row (op)
  (if (and (listp op) (>= (length op) 3))
      (second op)
      '()))

(defun dbms-row-op-new-row (op)
  (if (and (listp op) (>= (length op) 3))
      (third op)
      '()))

(defun dbms-row-op-rid (op)
  (let ((kind (dbms-row-op-kind op)))
    (if (eq kind 'insert)
        (dbms-row-id (dbms-row-op-new-row op))
        (dbms-row-id (dbms-row-op-old-row op)))))

(defun dbms-find-row-op-by-rid (ops rid)
  (if (null ops)
      '()
      (if (= (dbms-row-op-rid (car ops)) rid)
          (car ops)
          (dbms-find-row-op-by-rid (cdr ops) rid))))

(defun dbms-remove-row-op-by-rid (ops rid)
  (if (null ops)
      '()
      (if (= (dbms-row-op-rid (car ops)) rid)
          (dbms-remove-row-op-by-rid (cdr ops) rid)
          (cons (car ops) (dbms-remove-row-op-by-rid (cdr ops) rid)))))

(defun dbms-engine-compute-row-ops (old-rows new-rows)
  (let ((ops '()))
    (dolist (old-row old-rows)
      (let ((new-row (dbms-find-row-by-id new-rows (dbms-row-id old-row))))
        (if (null new-row)
            (setq ops (append ops (list (dbms-make-row-op 'delete old-row '()))))
            (if (equal (dbms-row-values old-row) (dbms-row-values new-row))
                nil
                (setq ops (append ops (list (dbms-make-row-op 'update old-row new-row))))))))
    (dolist (new-row new-rows)
      (if (null (dbms-find-row-by-id old-rows (dbms-row-id new-row)))
          (setq ops (append ops (list (dbms-make-row-op 'insert '() new-row))))
          nil))
    ops))

(defun dbms-engine-merge-row-op (ops step-op)
  (let* ((rid (dbms-row-op-rid step-op))
         (prev (dbms-find-row-op-by-rid ops rid))
         (kind (dbms-row-op-kind step-op)))
    (if (null prev)
        (append ops (list step-op))
        (let ((prev-kind (dbms-row-op-kind prev))
              (base-old (dbms-row-op-old-row prev)))
          (setq ops (dbms-remove-row-op-by-rid ops rid))
          (cond
           ((and (eq prev-kind 'insert) (eq kind 'update))
            (append ops (list (dbms-make-row-op 'insert '() (dbms-row-op-new-row step-op)))))
           ((and (eq prev-kind 'insert) (eq kind 'delete))
            ops)
           ((and (eq prev-kind 'update) (eq kind 'update))
            (append ops (list (dbms-make-row-op 'update base-old (dbms-row-op-new-row step-op)))))
           ((and (eq prev-kind 'update) (eq kind 'delete))
            (append ops (list (dbms-make-row-op 'delete base-old '()))))
           ((and (eq prev-kind 'delete) (eq kind 'insert))
            (append ops (list (dbms-make-row-op 'update base-old (dbms-row-op-new-row step-op)))))
           (t
            (append ops (list step-op))))))))

(defun dbms-engine-merge-row-ops (base-ops step-ops)
  (let ((merged base-ops))
    (dolist (step-op step-ops)
      (setq merged (dbms-engine-merge-row-op merged step-op)))
    merged))

(defun dbms-engine-table-delta-ops-or-empty (table-name staged-deltas)
  (let ((entry (dbms-find-staged-table-delta-pair table-name staged-deltas)))
    (if (and (listp entry) (>= (length entry) 2) (listp (second entry)))
        (second entry)
        '())))
...
(defun dbms-engine-apply-index-ops-for-entry (table-name idx-entry row-ops)
  (let* ((index-name (dbms-index-entry-name idx-entry))
         (index-columns (dbms-index-entry-columns idx-entry))
         (loaded (dbms-storage-load-index table-name index-name))
         (idx (if (dbms-btree-index-p loaded)
    ...
    (dolist (op row-ops)
      (if (dbms-error-p failed)
          nil
          (let ((kind (dbms-row-op-kind op))
                (old-row (dbms-row-op-old-row op))
                (new-row (dbms-row-op-new-row op)))
            (cond
             ((eq kind 'insert)
              (let ((next (dbms-storage-btree-insert idx
                                                     (dbms-storage-index-key-from-row new-row index-columns)
...
(defun dbms-engine-sync-table-indexes-ops-from-def (table-name table-def row-ops)
  (let ((indexes (dbms-table-indexes table-def))
        (failed '()))
    (dolist (idx-entry indexes)
      (if (dbms-error-p failed)
          nil
          (let ((saved (dbms-engine-apply-index-ops-for-entry table-name idx-entry row-ops)))
            (if (dbms-error-p saved)
                (setq failed saved)
                nil))))
    (if (dbms-error-p failed) failed t)))
...
(defun dbms-optimizer-count-keyword-in-toks (toks kw)
  (if (null toks)
      0
      (+ (if (dbms-token-is-keyword-p (car toks) kw) 1 0)
         (dbms-optimizer-count-keyword-in-toks (cdr toks) kw))))

(defun dbms-optimizer-count-operator-in-toks (toks op)
  (if (null toks)
      0
      (+ (if (dbms-token-is-operator-p (car toks) op) 1 0)
         (dbms-optimizer-count-operator-in-toks (cdr toks) op))))

(defun dbms-optimizer-where-complexity-score (where-toks)
  (if (null where-toks)
      0
      (+ (dbms-optimizer-count-keyword-in-toks where-toks "AND")
         (* 2 (dbms-optimizer-count-keyword-in-toks where-toks "OR"))
         (dbms-optimizer-count-keyword-in-toks where-toks "NOT")
         (dbms-optimizer-count-keyword-in-toks where-toks "IN")
         (dbms-optimizer-count-keyword-in-toks where-toks "IS")
         (dbms-optimizer-count-operator-in-toks where-toks "=")
         (dbms-optimizer-count-operator-in-toks where-toks "!=")
         (dbms-optimizer-count-operator-in-toks where-toks "<>")
         (dbms-optimizer-count-operator-in-toks where-toks ">")
         (dbms-optimizer-count-operator-in-toks where-toks ">=")
...
(defun dbms-optimizer-order-by-items-from-toks (order-toks)
  (if (null order-toks)
      0
      (+ 1 (dbms-optimizer-count-operator-in-toks order-toks ","))))

(defun dbms-optimizer-from-toks-table-refs (from-toks)
  ;; Extract table refs from FROM/JOIN token stream by positional cues.
  (let ((rest from-toks)
        (expect-table t)
        (out '()))
    (while (not (null rest))
      (let ((tok (car rest)))
        (if (and expect-table (dbms-token-is-kind-p tok 'identifier))
            (progn
              (setq out (append out (list (fourth tok))))
              (setq expect-table nil))
            nil)
        (if (dbms-token-is-keyword-p tok "JOIN")
            (setq expect-table t)
            nil)
        (if (dbms-token-is-operator-p tok ",")
            (setq expect-table t)
            nil))
      (setq rest (cdr rest)))
    out))

(defun dbms-optimizer-table-estimated-rows (catalog table-name)
  (let ((table-def (dbms-catalog-find-table catalog table-name)))
    (if (null table-def)
        0
        (let* ((stats (dbms-table-stats-from-def table-def))
               (n (dbms-assoc-get stats "row-count")))
          (if (and (numberp n) (>= n 0))
              n
...
(defun dbms-optimizer-insert-table-by-rows (catalog table-name sorted)
  (if (null sorted)
      (list table-name)
      (let* ((head (car sorted))
             (n1 (dbms-optimizer-table-estimated-rows catalog table-name))
             (n2 (dbms-optimizer-table-estimated-rows catalog head)))
        (if (< n1 n2)
            (cons table-name sorted)
            (cons head (dbms-optimizer-insert-table-by-rows catalog table-name (cdr sorted)))))))

(defun dbms-optimizer-join-order-by-rows (catalog tables)
  (if (null tables)
      '()
      (dbms-optimizer-insert-table-by-rows catalog
                                           (car tables)
                                           (dbms-optimizer-join-order-by-rows catalog (cdr tables)))))

(defun dbms-optimizer-join-cost (catalog order)
  ;; nested-loop style rough model: cumulative intermediate size growth
  (let ((rest order)
        (prefix 1)
        (cost 0))
    (while (not (null rest))
      (let ((n (dbms-optimizer-table-estimated-rows catalog (car rest))))
        (if (< n 1) (setq n 1) nil)
        (setq cost (+ cost (* prefix n)))
        (setq prefix (* prefix n)))
      (setq rest (cdr rest)))
    cost))

(defun dbms-optimizer-plan-join-details (catalog from-toks where-toks order-toks limit-n)
  (let* ((tables (dbms-optimizer-from-toks-table-refs from-toks))
         (naive-order tables)
         (optimized-order (dbms-optimizer-join-order-by-rows catalog tables))
         (naive-cost (dbms-optimizer-join-cost catalog naive-order))
         (optimized-cost (dbms-optimizer-join-cost catalog optimized-order))
         (chosen (if (<= optimized-cost naive-cost) optimized-order naive-order)))
    (list (list "query-kind" "select-wiki")
          (list "tables" tables)
          (list "join-count" (if (<= (length tables) 0) 0 (- (length tables) 1)))
          (list "naive-order" naive-order)
          (list "optimized-order" optimized-order)
          (list "chosen-order" chosen)
          (list "join-cost-naive" naive-cost)
          (list "join-cost-optimized" optimized-cost)
          (list "where-complexity" (dbms-optimizer-where-complexity-score where-toks))
          (list "order-by-items" (dbms-optimizer-order-by-items-from-toks order-toks))
          (list "limit" (if (null limit-n) '() limit-n))
          (list "optimizer-version" "MT-003"))))
...
(defun dbms-row-lock-key-from-row (table-def row)
  (let ((pk-cols (dbms-table-primary-key-columns table-def)))
    (if (= (length pk-cols) 1)
        (let ((v (dbms-row-get-value row (first pk-cols))))
          (if (eq v 'NULL)
              ""
              (format nil "~A" v)))
        "")))

(defun dbms-engine-acquire-row-x-lock-if-needed (table-name table-def row)
  (if (dbms-tx-active-p)
      (if (dbms-row-lock-enabled-p)
          (let ((row-key (dbms-row-lock-key-from-row table-def row)))
            (if (string= row-key "")
                t
                (dbms-row-lock-acquire (dbms-tx-state-tx-id *dbms-tx-state*) table-name row-key 'X)))
          t)
      t))
  ...
  (let ((rest-cols columns)
              ...
              (if (dbms-error-p ev)
                  ...
                  (let ((et (first ev))
                    ...
                    (if (dbms-type-compatible-p target-type et)
                        (setq pairs (cons (list col-name evv) pairs))
    ...
    (while (and (null failed) (not (null rest-cols)))
      (let* ((col (car rest-cols))
             (name (dbms-column-def-name col))
             (ty (dbms-column-def-type col)))
        (if (null (dbms-row-values-get pairs name))
            (if (eq ty 'BIGSERIAL)
                (setq pairs (cons (list name (dbms-next-serial-for-column rows name)) pairs))
                (if (dbms-col-default-now-p col)
                    (setq pairs (cons (list name (dbms-current-timestamptz)) pairs))
                    (setq pairs (cons (list name 'NULL) pairs))))
            nil))
...
(defun dbms-list-append1 (xs x)
  (dbms-reverse (cons x (dbms-reverse xs))))
...
(defun dbms-table-stats (rows columns)
  (let ((col-rest columns)
    ...
    (while (not (null col-rest))
      (setq col-stats (cons (dbms-column-stats rows (dbms-column-def-name (car col-rest))) col-stats))
      (setq col-rest (cdr col-rest)))
    (list (list "row-count" (length rows))
          (list "columns" (dbms-reverse col-stats)))))
            ...
            (if (null table-def)
                ...
                (let ((report (dbms-storage-vacuum-table-pages name)))
                  (if (dbms-error-p report)
                      (setq failed report)
                      (setq rows (cons (car (dbms-engine-vacuum-report-row report)) rows))))))
          (setq rest (cdr rest)))
        (if (dbms-error-p failed)
            (dbms-make-result 'error failed)
...
(defun dbms-engine-explain-select-wiki (catalog stmt)
  (let* ((payload (third stmt))
         (from-toks (dbms-assoc-get payload "from"))
         (where-toks (dbms-assoc-get payload "where"))
         (order-toks (dbms-assoc-get payload "order-by"))
         (limit-n (dbms-assoc-get payload "limit"))
         (plan (dbms-optimizer-plan-join-details catalog from-toks where-toks order-toks limit-n)))
    (dbms-make-result-rows '("item" "value") plan)))
...
(defun dbms-engine-explain (catalog stmt)
  (let* ((inner (dbms-assoc-get (third stmt) "statement"))
    ...
    (if (eq kind 'select)
        ...
        (if (eq kind 'select-wiki)
            (dbms-engine-explain-select-wiki catalog inner)
            (dbms-make-result-error 'dbms/not-implemented "EXPLAIN supports SELECT/SELECT-WIKI" kind)))))
        ...
        (let* ((state (dbms-engine-load-table-rows table-name))
          ...
          (if (dbms-error-p pairs)
              ...
              (let* ((new-row (dbms-make-row (dbms-next-row-id rows) pairs))
                     (new-rows (dbms-list-append1 rows new-row))
                ...
                (if (not (null bad-col))
                    ...
                    (let* ((ordered
                      ...
                      (if (eq limited 'ORDER-BY-COLUMN-MISSING)
                          ...
                          (progn
                            (dolist (r limited)
                              (setq projected (cons (dbms-row-project r out-cols) projected)))
                            (dbms-make-result-rows out-cols (dbms-reverse projected))))))))))))
  ...
  (let* ((payload (third stmt))
    ...
    (if (null table-def)
        ...
        (let* ((state (dbms-engine-load-table-rows table-name))
               (table-cols (dbms-table-def-columns table-def)))
          (if (dbms-error-p state)
              (dbms-make-result 'error state)
              (if (or (null state) (< (length state) 3))
                  (dbms-make-result-error 'dbms/invalid-representation "invalid table state" table-name)
                  (let* ((rows (third state))
                      ...
                      (while (and (null failed) (not (null sr)))
                        (let* ((assignment (car sr))
                               (col-name (if (and (listp assignment) (not (null assignment)))
                                             ...
                                             '())))
                          (if (or (null col-name)
                                  (not (stringp col-name))
                                  (null (dbms-find-column-def table-cols col-name)))
                              (setq failed (dbms-make-error 'dbms/column-not-found "column not found" col-name))
                              nil))
                        (setq sr (cdr sr))))
                    (let ((rest rows))
                      (while (and (null failed) (not (null rest)))
                        (let* ((row (car rest))
                          ...
                          (if (dbms-error-p ok)
                              (setq failed ok)
                              (if ok
                                  (let ((new-row (dbms-engine-build-updated-row
                                                  table-name table-def table-cols row set-pairs auto-touch-updated-at)))
                    ...
                    (if (dbms-error-p failed)
                        (dbms-make-result 'error failed)
                        (let* ((updated-rows (dbms-reverse updated))
                               (touched-rows (dbms-reverse touched))
                               (valid (dbms-validate-rows updated-rows table-def catalog)))
                          (if (dbms-error-p valid)
                              (dbms-make-result 'error valid)
      ...
      (let* ((child-name (first p))
        ...
        (dolist (c cons-list)
          (if (and (eq (dbms-constraint-kind c) 'foreign-key)
              ...
              (setq out (cons (list child-name child-def c) out))
              nil))))
    (dbms-reverse out)))
...
(defun dbms-plan-on-delete-child-rows (rows deleted-rows fk-spec)
  (let ((new-rows '()))
    (dolist (r rows)
      (let ((res (dbms-apply-on-delete-to-child-row r deleted-rows fk-spec)))
        (if (string= (first res) "KEEP")
            (setq new-rows (cons (second res) new-rows))
            nil)))
    (dbms-reverse new-rows)))

(defun dbms-build-on-delete-plans (catalog target-table deleted-rows)
      ...
      (if (dbms-error-p failed)
          ...
          (let* ((child-name (first entry))
            ...
            (if (dbms-error-p state)
                (setq failed state)
                (if (or (null state) (< (length state) 3))
                    (setq failed (dbms-make-error 'dbms/invalid-representation
                                                  "invalid table state"
                                                  child-name))
                    (let* ((rows (third state))
                           (new-rows (dbms-plan-on-delete-child-rows rows deleted-rows fk-spec))
                           (valid (dbms-validate-rows new-rows child-def catalog)))
                       (if (dbms-error-p valid)
                           (setq failed valid)
                           (setq plans (cons (list child-name child-def rows new-rows) plans)))))))))
    (if (dbms-error-p failed)
        failed
        (dbms-reverse plans))))

(defmacro dbms-guard-valid-table-state (state table-name &rest body)
  `(if (dbms-error-p ,state)
       (dbms-make-result 'error ,state)
       (if (or (null ,state) (< (length ,state) 3))
           (dbms-make-result-error 'dbms/invalid-representation "invalid table state" ,table-name)
...
(defun dbms-apply-on-delete-plans (plans)
  (let ((failed '()))
    (dolist (plan plans)
      (if (dbms-error-p failed)
          nil
                         ...
                         (if ok
                             (let ((lock-r (dbms-engine-acquire-row-x-lock-if-needed table-name table-def row)))
                               (if (dbms-error-p lock-r)
                                   ...
                                   (progn
                                     (setq affected (+ affected 1))
                                     (setq deleted (cons row deleted)))))
                             (setq kept (cons row kept)))))))
             (if (dbms-error-p failed)
                 (dbms-make-result 'error failed)
                 (let* ((deleted-rows (dbms-reverse deleted))
                        (kept-rows (dbms-reverse kept))
                        (plans (dbms-build-on-delete-plans catalog table-name deleted-rows)))
                   (if (dbms-error-p plans)
                       (dbms-make-result 'error plans)
                       (let ((saved (dbms-engine-persist-table-change-stable
                                     table-name table-def rows kept-rows)))
                         (if (dbms-error-p saved)
                             (dbms-make-result 'error saved)
                             (let ((prop (dbms-apply-on-delete-plans plans)))
              ...
              (let ((entry (dbms-find-staged-table-state-pair name *dbms-tx-staged-table-states*)))
                (if (null entry)
                    ...
                    (let* ((state (second entry))
                           ...
                           (row-ops (dbms-engine-table-delta-ops-or-empty name *dbms-tx-staged-table-deltas*))
                           (saved (if (null row-ops)
                                      (dbms-engine-save-table-rows-commit-stable name old-rows (third state))
                                      (dbms-storage-save-table-rows-ops name (third state) row-ops))))
                      (if (dbms-error-p saved)
                          (setq failed saved)
                          nil))))))
              ...
              (let ((table-def (dbms-catalog-find-table *dbms-tx-staged-catalog* name)))
                (if (null table-def)
                    ...
                    (let* ((entry (dbms-find-staged-table-state-pair name *dbms-tx-staged-table-states*))
                      ...
                      (if (dbms-error-p state)
                          (setq failed state)
                          (let ((updated-indexes (if (null row-ops)
                                                     (dbms-engine-sync-table-indexes-commit-stable
                                                      name table-def old-rows (third state))
                                                     (dbms-engine-sync-table-indexes-ops-from-def
                                                      name table-def row-ops))))
                            (if (dbms-error-p updated-indexes)
...
(defun dbms-engine-restore-storage-root! (root)
  (if (or (null root) (not (stringp root)))
      (setenv "DBMS_STORAGE_ROOT" "")
      (setenv "DBMS_STORAGE_ROOT" root)))

(defun dbms-engine-bootstrap-follower-from-dump-payload (follower-root payload)
  (if (not (dbms-dump-payload-p payload))
      (dbms-make-error 'dbms/invalid-representation "invalid dump payload" payload)
      (let* ((old-root (getenv "DBMS_STORAGE_ROOT"))
             (catalog (third payload))
        ...
        (if (dbms-error-p failed)
            nil
            (let ((saved-catalog (dbms-catalog-save catalog)))
              (if (dbms-error-p saved-catalog)
                  (setq failed saved-catalog)
                  nil)))
        (if (dbms-error-p failed)
            ...
            (dolist (entry states)
              (if (dbms-error-p failed)
                  ...
                  (let ((name (first entry))
                    ...
                    (if (or (not (stringp name))
                            (not (dbms-table-state-p state)))
                        (setq failed (dbms-make-error 'dbms/invalid-representation "invalid table-state entry in dump payload" entry))
                        (let ((saved (dbms-storage-save-table-rows name (third state))))
                          (if (dbms-error-p saved)
                              (setq failed saved)
                              (let ((table-def (dbms-catalog-find-table catalog name)))
...
(defun dbms-engine-run-recovery-on-root (target-root)
  (let* ((old-root (getenv "DBMS_STORAGE_ROOT"))
         (r '()))
    (dbms-engine-restore-storage-root! target-root)
    (setq r (dbms-storage-recover-from-wal))
    (dbms-engine-restore-storage-root! old-root)
    r))
...
(defun dbms-admin-replication-set-mode (mode)
  (if (dbms-tx-active-p)
      ...
      (let ((mode-s (if (stringp mode) mode (format nil "~A" mode))))
        (if (or (null mode-s) (not (stringp mode-s)))
            (dbms-make-result-error 'dbms/invalid-representation "mode must be ASYNC or SYNC" mode)
            (let ((saved (dbms-storage-repl-set-mode mode-s)))
              (if (dbms-error-p saved)
                  (dbms-make-result 'error saved)
                  (dbms-make-result-ok saved)))))))

(defun dbms-admin-replication-status ()
  (dbms-make-result-ok (dbms-storage-load-repl-meta)))

(defun dbms-admin-replication-register-follower (follower-id follower-root)
  (if (dbms-tx-active-p)
      (dbms-make-result-error 'dbms/tx-already-active "cannot register follower while transaction is active" (list follower-id follower-root))
      (let* ((payload (dbms-admin-build-dump-payload))
             (registered '())
             (bootstrapped '())
             (ship-report '())
        ...
        (if (dbms-error-p failed)
            nil
            (progn
              (setq registered (dbms-storage-repl-register-follower follower-id follower-root))
              (if (dbms-error-p registered)
                  (setq failed registered)
                  nil)))
        (if (dbms-error-p failed)
            nil
            (progn
              (setq bootstrapped (dbms-engine-bootstrap-follower-from-dump-payload follower-root payload))
              (if (dbms-error-p bootstrapped)
                  (setq failed bootstrapped)
                  nil)))
        (if (dbms-error-p failed)
            nil
            (progn
              (setq ship-report (dbms-storage-repl-ship-wal-to-root follower-root))
              (if (dbms-error-p ship-report)
                  (setq failed ship-report)
                  nil)))
        (if (dbms-error-p failed)
            (dbms-make-result 'error failed)
            (let* ((max-lsn (dbms-assoc-get (cdr ship-report) "max-lsn"))
                   (recovery (dbms-engine-run-recovery-on-root follower-root))
                   (applied-lsn (if (numberp max-lsn) max-lsn 0))
                   (sync-state (if (dbms-error-p recovery) "ERROR" "ASYNC-CAUGHT-UP"))
                   (updated (dbms-storage-repl-update-follower-progress follower-id applied-lsn applied-lsn sync-state)))
              (if (dbms-error-p recovery)
                  (dbms-make-result 'error recovery)
                  (if (dbms-error-p updated)
                      (dbms-make-result 'error updated)
                      (dbms-make-result-ok updated))))))))

(defun dbms-admin-replication-ship ()
  (if (dbms-tx-active-p)
      (dbms-make-result-error 'dbms/tx-already-active "cannot run replication shipping while transaction is active" *dbms-tx-state*)
      (let* ((meta (dbms-storage-load-repl-meta))
             (followers (dbms-storage-repl-followers meta))
             (mode (dbms-storage-repl-mode meta))
             (failed '())
             (reports '()))
        (dolist (f followers)
          (if (dbms-error-p failed)
              nil
              (let* ((fid (dbms-repl-follower-id f))
                     (root (dbms-repl-follower-root f))
                     (ship (dbms-storage-repl-ship-wal-to-root root)))
                (if (dbms-error-p ship)
                    (setq failed ship)
                    (let* ((max-lsn (dbms-assoc-get (cdr ship) "max-lsn"))
                           (applied-before (fifth f))
                           (updated (dbms-storage-repl-update-follower-progress fid
                                                                                max-lsn
                                                                                applied-before
                                                                                (if (string= mode "SYNC")
                                                                                    "SYNC-PENDING-APPLY"
                      ...
                      (if (dbms-error-p updated)
                          (setq failed updated)
                          (setq reports (append reports (list ship)))))))))
        (if (dbms-error-p failed)
            (dbms-make-result 'error failed)
            (dbms-make-result-ok reports)))))

(defun dbms-admin-replication-apply-follower (follower-id)
  (if (dbms-tx-active-p)
      (dbms-make-result-error 'dbms/tx-already-active "cannot apply follower while transaction is active" follower-id)
      (let* ((meta (dbms-storage-load-repl-meta))
             (f (dbms-storage-repl-find-follower (dbms-storage-repl-followers meta) follower-id)))
        (if (null f)
            (dbms-make-result-error 'dbms/invalid-representation "follower not found" follower-id)
            (let* ((root (dbms-repl-follower-root f))
                   (applied (dbms-engine-run-recovery-on-root root))
                   (max-lsn (dbms-storage-wal-max-lsn))
                   (mode (dbms-storage-repl-mode meta))
                   (next-state (if (dbms-error-p applied)
                                   "ERROR"
                                   (if (string= mode "SYNC") "SYNC-ACK" "ASYNC-ACK")))
                   (updated (if (dbms-error-p applied)
                                '()
                                (dbms-storage-repl-update-follower-progress follower-id max-lsn max-lsn next-state))))
...
(defun dbms-admin-replication-failover (follower-id)
  ;; Manual failover: switch current node view to follower root and re-init.
  (if (dbms-tx-active-p)
      (dbms-make-result-error 'dbms/tx-already-active "cannot failover while transaction is active" follower-id)
      (let* ((meta (dbms-storage-load-repl-meta))
             (f (dbms-storage-repl-find-follower (dbms-storage-repl-followers meta) follower-id)))
        (if (null f)
            (dbms-make-result-error 'dbms/invalid-representation "follower not found" follower-id)
            (let* ((old-root (dbms-storage-root))
                   (new-root (dbms-repl-follower-root f))
                   (ship (dbms-storage-repl-ship-wal-to-root new-root))
                   (applied (if (dbms-error-p ship) ship (dbms-engine-run-recovery-on-root new-root))))
              (if (dbms-error-p ship)
