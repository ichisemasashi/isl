...
(defun dbms-cli-parse-and-exec (sql-text)
    ...
    (if (dbms-error-p ast)
        ...
        (let ((stmts (dbms-ast-statements ast)))
          (if (or (null stmts) (not (= (length stmts) 1)))
              ...
              (dbms-engine-dispatch (dbms-catalog-load) (first stmts)))))))

(defun main ()
  (let* ((sql-text (dbms-read-stdin-text))
         (result (if (string= sql-text "")
                     (dbms-make-result-error 'dbms/no-input "stdin is empty" '())
                     (dbms-cli-parse-and-exec sql-text))))
