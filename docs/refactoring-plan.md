...
### R7. core ↔ runtime 重複の削減 ✅完了
同名定義 51 個を本体テキストで厳密比較し、**完全一致した 22 個のみ**を
`isl.core` に集約・export し runtime 側の複製を削除（runtime は既に `(use isl.core)`）。
- 集約した 22 個: `gauche-*` ラッパ 17（TLS/network/thread/mutex）、
  `make-go-signal`/`go-signal?`/`go-signal-tag`、`score<?`/`score=?`。
  いずれも純粋ユーティリティで rt-value 非依存・predicate 差異なし。
- **据え置いた差異あり 26 個**: 実装が実際に異なる。例:
  `closure-body` は core が `(vector-ref c 2)`／runtime が `(vector-ref c 3)`
  （クロージャの record レイアウトが別物）、`validate-params` は戻り値が
  `params`／`#t` と異なる。`parse-params`・`bind-params!`・CLOS 補助
  （`ensure-generic-function!`/`select-generic-method`/`method-applicability-score`）・
  `handler-case-tag-match?` 等は rt-value ラップや評価モデルの違いを反映。
  これらの共通化は「値表現の統一」という別次元の大改修になり高リスクのため対象外。
- リスク評価: 完全一致のみを移動したため挙動不変。thread/mutex を
  interpreter・JIT 双方で実行し dedup 後の経路を確認済み。

### R8. C ランタイムの分割（llvm_runtime.c 2301 行）✅完了
`bin/islc` / `bin/islc-aot` は `clang -x ir … -x c …/llvm_runtime.c` で**単一翻訳
単位**としてビルドする。ビルドを変えずに保守性を上げるため、アンブレラとなる
`llvm_runtime.c`（179 行: include・型定義・前方宣言）が 8 断片を**定義順を保って**
`#include` する unity 方式を採用。前処理結果は分割前と同一（行範囲の移動のみ）。
- 断片: `llvm_runtime/{numbers,sequences,control,conditions,clos,dynamic_convert,
  format,api}.c`（110〜611 行）。各断片は波括弧バランスを検証。
- 各断片は単独コンパイル不可（アンブレラ経由でのみ使用）とヘッダコメントに明記。
- 検証: native-gap-probe 49/49、compiler 45/46、`--run` / AOT バイナリ / JIT を実機確認。
...
| ✅ R7 | core/runtime の**完全一致**ユーティリティ 22 個を isl.core に集約 | 重複 22 関数を単一の出所へ |
| ✅ R8 | llvm_runtime.c を論理単位 8 断片へ分割（unity `#include`） | 2301 行 → 179 行アンブレラ + 8 断片 |
