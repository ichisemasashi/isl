...
## 追加ライブラリ
リポジトリ同梱の補助ライブラリとして [lib/os-utils.lsp](/Volumes/SSD-PLU3/work/LISP/islisp/isl/lib/os-utils.lsp) を追加しています。`load` して使う想定で、次のような OS 操作 API を提供します。

```lisp
(load "/Volumes/SSD-PLU3/work/LISP/islisp/isl/lib/os-utils.lsp")
(os-mkdir-p "/tmp/example")
(os-chmod "644" "/tmp/example/file.txt")
(os-cat-file "/tmp/example/file.txt")
(os-wc-c "/tmp/example/file.txt")
(os-file-mime-type "/tmp/example/file.txt")
(os-ls "/tmp/example")
(os-date-utc-iso8601)
```
