...
- 基本組み込み関数: 算術・比較・数値(`mod`/`floor`/`ceiling`/`truncate`/`round`)・数値述語(`zerop`/`plusp`/`minusp`/`evenp`/`oddp`)・リスト・述語・高階関数(`mapcar`/`reduce`/`remove-if`)・文字列(`string=`/`string-concat`/`substring`/`length`)・`apply`/`funcall`・`format` など
